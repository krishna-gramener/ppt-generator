...
$posterForm.addEventListener("submit", async (e) => {
  ...
  await applyChanges(responseContent);
...
async function drawImage({ prompt, aspectRatio }) {
  ...
  const data = await fetch("https://llmfoundry.straive.com/vertexai/google/models/imagen-4.0-generate-preview-05-20:predict", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}:postergen` },
...
$enhanceImageBtn.addEventListener("click", async () => {
  ...
  try {
    const responseContent = await fetch("https://llmfoundry.straive.com/openai/v1/chat/completions", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}:postergen` },
      body: JSON.stringify({
        model: "gpt-4.1-mini",
        messages: [
          { role: "system", content: `Update only required components and properties\n\n${userPrompt}\n\n` + responseSchema },
          { role: "user", content: `Poster for ${logo.name}\n\nCOMPONENTS:\n${getComponentsPrompt()}` },
        ],
      }),
    })
    ...
    await applyChanges(responseContent, { enhance: true });
    $downloadContainer.classList.remove("d-none");
    ...
    document.querySelector(`#msg-${msgId} .spinner-border`)?.remove();
    addMessageToConversation("assistant", "Sorry, there was an error processing your request. Please try again.");
  }
});
...
function applyChanges(responseContent, { enhance } = { enhance: false }) {
  ...
  for (const [name, config] of Object.entries(params)) {
    ...
    for (const [prop, value] of Object.entries(config)) {
      if (prop === "@text") $el.textContent = value;
        ...
        let imagePromise;
        if (enhance) imagePromise = enhanceImage({ prompt: value, originalImage: $el.src });
        else {
          $el.src = "loading.svg";
          imagePromise = drawImage({ prompt: value, aspectRatio: getClosestAspectRatio($el.width / $el.height) });
        }
        imagePromises.push(imagePromise.then(async (src) => ($el.src = src))); // Add to promises array
      } else if (prop === "styles") {
        typeof value === "string" ? ($el.style.cssText = value) : Object.assign($el.style, value);
